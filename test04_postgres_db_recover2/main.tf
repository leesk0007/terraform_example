...
# 1. 인프라 생성 후 ansible play book을 실행 가능한 시간만큼 대기한다.
...
resource "terraform_data" "ansible_run" {
    ...
    triggers_replace = {
        ...
        always_run = "${timestamp()}" # 실행할때마다 시간이 달라지므로 무조건 플레이북이 실행된다
    }

    # 아래의 작업이 성공한 기억이 있으면 다시 또 실행되지 않는다.
    provisioner "local-exec" {
        # db 설치하고 파일 copy하고 권한 변경이 잦고하면 에러가 날수 있기에 안정적 실행을 위해 필요
        # ANSIBLE_SSH_PIPLINING=1 라는 임시 환경 변수를 추가한다.
        # 해당 환경변수는 ansible playbook이 실행될때에 한해서 ansible이 사용하는 일시적인 환경변수다
        command = "ANSIBLE_SSH_PTPELINING=1 ansible-playbook site.yml"
    }
}